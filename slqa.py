...
import layers
from util import InputEmbeddings, masked_softmax
...
class Fusion(nn.Module):
    ...
    def forward(self, x, y):
        out = torch.cat([x, y, x * y, x - y], dim=2)
        return self.tanh(self.linear(out))
...
class FusionLayer(nn.Module):
    ...
    # pylint: disable=W0221
    def forward(self, x, y):
        cat = torch.cat([x, y, x * y, x - y], dim=2)
        gated = self.sigmoid(self.linear_g(cat))
        fusion = self.tanh(self.linear_f(cat))
...
class FusedSelfAttention(nn.Module):
    ...
    This is a bilinear self-alignment attention function. The implementation is based on AllenNLP's
    https://github.com/allenai/allennlp/blob/master/allennlp/\
            modules/matrix_attention/bilinear_matrix_attention.py
    """
...
class SLQA(nn.Module):
    ...
    def forward(self, pw_idxs: torch.Tensor,
        ...
        (p_tilde, q_tilde) = self.att(p_enc, q_enc,
                                      p_mask, q_mask)    # 2 x (batch_size, p_len, 2*hidden_size)
